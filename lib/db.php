...
class mysql {
	...
	function fetch( $callback ) {
		while( $res = $this->statment->fetch(PDO::FETCH_ASSOC) ) {
			$callback( $res );
		}
	}
	...
	public function pagination( $get, $in_page = 5 ) {
		...
		$this->paginate["n"] = ( isset( $get["npage"] ) && ! empty( $get['npage'] ) ) ? (int)$get["npage"] : $in_page;
		$this->paginate["p"] = isset( $get["page"] ) ? (int)$get["page"] : 1;
